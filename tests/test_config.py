...
def test_load_app_config_reads_anthropic_section(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("CODE_FLASH_MODEL", raising=False)
    monkeypatch.delenv("CODE_FLASH_MAX_TOKENS", raising=False)

    config_path = tmp_path / "code-flash.toml"
...
def test_load_app_config_cli_overrides_env_and_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("CODE_FLASH_MODEL", "claude-opus-4")
    monkeypatch.setenv("CODE_FLASH_MAX_TOKENS", "1234")
...
def test_load_app_config_reads_openai_section(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("CODE_FLASH_PROVIDER", raising=False)
...
def test_openai_env_wins_when_provider_is_openai(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("CODE_FLASH_PROVIDER", "openai")
    monkeypatch.setenv("OPENAI_API_KEY", "openai-env-key")
    monkeypatch.setenv("OPENAI_BASE_URL", "https://openai.env")
    monkeypatch.setenv("CODE_FLASH_MODEL", "gpt-4.1")
    monkeypatch.setenv("CODE_FLASH_BUDDY_MODEL", "gpt-4.1-mini")
